...
2. Read repo guidance first when available: `AGENTS.md`, `README*`, `docs/**/*.md`, architecture notes, findings, roadmap, and runbooks.
3. Map the relevant code area with `rg --files`, focused searches, nearby tests, and relevant config.
4. Look for:
   - patch-over-fix behavior
   - duplicated logic
   - hidden state or sidecars
   - unclear source of truth
   - over-broad abstractions
   - hardcoded assumptions
   - stale generated output or checked-in artifacts
   - fragile path, identity, or lifecycle logic
